...
class TestColoredFormatter:
    ...
    @pytest.mark.parametrize(
        "case_desc, use_color, expect_color",
        [
            ("enabled", True, True),
            ("disabled", False, False),
        ],
    )
    def test_color_toggle(self, case_desc, use_color, expect_color):
        formatter = ColoredFormatter(
            fmt="%(levelname)s - %(message)s", use_color=use_color
        )
    ...
    def test_colored_fmt_levelname_restore(self):
    ...
    def test_debug_level_no_color(self):
        """DEBUG level (unmapped) does not crash or apply colors."""
...
class TestSetupLogging:
    ...
    @pytest.mark.parametrize(
        "case_desc, force_color, expect_color",
        [
            ("force color on", "1", True),
            ("default no color", None, False),
        ],
    )
    def test_setup_logging_colors(
        self, monkeypatch, case_desc, force_color, expect_color
    ):
        if force_color is not None:
            monkeypatch.setenv("FORCE_COLOR_LOGGING", force_color)
        else:
            monkeypatch.delenv("FORCE_COLOR_LOGGING", raising=False)
    ...
    @pytest.mark.parametrize("logger_name", ["asyncio", "urllib3"])
    def test_setup_logging_logger_level(self, monkeypatch, logger_name):
        monkeypatch.delenv("FORCE_COLOR_LOGGING", raising=False)
        setup_logging()
