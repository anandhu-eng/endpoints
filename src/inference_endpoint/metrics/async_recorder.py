    ...
    async def process(self, records: list[EventRecord]) -> None:
        ...
        for record in records:
            ...
            if record.data is not None:
                data_bytes = msgspec.json.encode(record.data)
...
def _subscriber_main(
    ...
    async def _run():
        ...
        # Parse the publisher address to extract socket_dir and path.
        # Publisher address format for IPC: "ipc://<socket_dir>/<path>"
        # urlparse puts the full filesystem path in parsed.path for non-registered
        # URI schemes like "ipc" (netloc is empty).
        parsed = urlparse(publisher_address)
        scheme = parsed.scheme or "ipc"

        if scheme == "ipc":
            # parsed.path contains the full filesystem path, e.g. "/tmp/zmq_xxx/ev_pub_abc"
            full_path = parsed.path
            socket_dir = str(Path(full_path).parent)
            socket_name = Path(full_path).name
            zmq_ctx = ManagedZMQContext(io_threads=1, socket_dir=socket_dir)
        else:
            # TCP: path is host:port in netloc
            socket_name = parsed.netloc or parsed.path
...
class AsyncEventRecorder:
    ...
    Usage (context manager):
        with AsyncEventRecorder(session_id, publisher.bind_address) as writer:
            # ... benchmark runs, publisher publishes events ...
            pass
